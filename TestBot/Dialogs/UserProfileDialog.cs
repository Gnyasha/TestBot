...
namespace TestBot.Dialogs
{
    public class UserProfileDialog : ComponentDialog
    {
        ...
        private async Task<DialogTurnResult> MaritalStatusConfirmStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            // WaterfallStep always finishes with the end of the Waterfall or with another dialog; here it is a Prompt Dialog.
            return await stepContext.PromptAsync(nameof(ConfirmPrompt), new PromptOptions { Prompt = MessageFactory.Text("Would you like to give your marital status?") }, cancellationToken);
        }
        private async Task<DialogTurnResult> MaritalStatusAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            if ((bool)stepContext.Result)
            {
                // User said "yes" so we will be prompting for the marital status.
                // WaterfallStep always finishes with the end of the Waterfall or with another dialog; here it is a Prompt Dialog.

                return await stepContext.PromptAsync(nameof(ChoicePrompt),
                 new PromptOptions
                 {
                     Prompt = MessageFactory.Text("Please enter your marital status."),
                     Choices = ChoiceFactory.ToChoices(new List<string> { "Married", "Single", "Divorced", "Widowed" }),
                 }, cancellationToken);
            }
            else
            {
        ...
        private async Task<DialogTurnResult> SummaryStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            if ((bool)stepContext.Result)
            {
                ...
                userProfile.MaritalStatus = (string)stepContext.Values["maritalStatus"];
                ...
                msg += ".";
                if (userProfile.MaritalStatus.Length > 3)
                {
                    msg += $" You are {userProfile.MaritalStatus}";
                }
                await stepContext.Context.SendActivityAsync(MessageFactory.Text(msg), cancellationToken);
